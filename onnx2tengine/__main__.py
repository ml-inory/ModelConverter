...
def parse_args():
    import argparse
    parser = argparse.ArgumentParser(description='ONNX to Tengine model Converter')
    parser.add_argument('--onnx', help='onnx model file path', required=True)
    parser.add_argument('--tmfile', help='tengine model file path', required=True)
...
def convert(onnx_file, tmfile):
    rename_share_weights(onnx_file)

    os.system(f'convert_tool -f onnx -m {onnx_file} -o {mnn_file}')
    
def main():
    args = parse_args()
    convert(args.onnx, args.tmfile)
