...
* Author: rzyang
* Date: 2020/08/26
* Description: Convert mmcls PyTorch model to ONNX and check result.
*******************************************************************************
'''
__all__ = ['convert', ]
...
def parse_args():
    import argparse
    parser = argparse.ArgumentParser(description='MMClassification PyTorch model to ONNX model Converter')
    parser.add_argument('--config', help='MMClassification config file path', required=True)
    parser.add_argument('--checkpoint', help='MMClassification .pth file path', required=False, default=None)
    parser.add_argument('--onnx', help='onnx model file path', required=True)
    parser.add_argument('--input_size', help='input size of model, format: height width channel', type=int, required=True, nargs='*')
...
def convert(config_file, ckp_file, onnx_file, input_size=(224,224,3), device='cpu'):
    ...
    # Convert to onnx
    with torch.no_grad():
        torch.onnx.export(mmcls_model, x, onnx_file, verbose=False, input_names=['input'], opset_version=9) # 9
